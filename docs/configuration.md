...
| `activityLog` | Path to append the activity log to — the lines the TUI shows: request rows, account rotation, upstream errors, 429 failover. Unset (default) keeps them in the TUI's 200-line ring buffer and nowhere else, and the TUI replaces `console.log`, so a restart loses them: when a proxy problem has to be diagnosed after the fact, this file is the only record. `~` is expanded; the file is created `0600` and appended to, never rotated, so point it somewhere you are willing to prune. `--activity-log FILE` overrides it for one launch. Read at startup |
| `autoRestart` | Drain and restart the proxy on its own once a newer build is waiting; `false` (default) leaves every restart to you. What counts as waiting depends on the install: a **global npm install** compares the version in the installed package — the file `npm install -g` rewrites underneath the running process — against the one it is executing, so this applies what [auto-update](usage.md#auto-update) has already fetched; a **git checkout** watches its own `HEAD` sha, so a `git pull` in the deploy clone is the deploy. Neither fires while the fleet is busy: the restart waits until nothing is in flight and no session is counted active, and goes ahead regardless after 30 minutes, so a busy fleet cannot postpone it forever. The restart itself is a [drain](usage.md#drain-and-restart) — in-flight requests are served to the end and pooled sockets are retired cooperatively — and needs something to relaunch the process (`teamclaude server --supervise`); without one the server says so at startup and leaves the setting off for that run. Read at startup |
...
| `sidecars` | Optional list of processes the server supervises — spawned on start, respawned with exponential backoff on crash, killed on shutdown. Objects: `{ name, command: [bin, ...args], env? }`. Built for a local [translating sidecar](openai.md) a third-party account routes to; status (pid, restarts, last stderr lines) appears in `status --json` under `sidecars` |
| `customModels` | Optional list of third-party models to register with Claude Code at launch — see [OpenAI models](openai.md). Objects: `{ model, label?, description?, contextTokens? }`. `run` injects `/model` picker rows (`--settings`), and one dispatchable subagent per model (`--agents`) if `customModelAgents` below is on; both `run` and `env` set `CLAUDE_CODE_MAX_CONTEXT_TOKENS` to the largest `contextTokens` and register the first entry via `ANTHROPIC_CUSTOM_MODEL_OPTION` |
| `customModelAgents` | Set to `true` to have `run` inject one plain subagent per custom model (`--agents`). **Off by default**: those agents pin no effort, so a dispatch runs the model at whatever effort the parent session is on, and `--agents` outranks every agent file on disk — including the effort-pinned ones from the [`rikclaude-agents` plugin](agents.md), which cover the same models and do pin it. Turn it on for a fleet that defines no agents of its own. Picker rows and env vars are unaffected |
| `autoUpdate` | Set to `false` to disable the background [self-update](usage.md#auto-update) check |
| `autoRedeemResets` | Let a Codex account whose weekly window has run out spend one of its [free rate-limit reset credits](openai.md#free-rate-limit-reset-credits) to clear it. **`false` by default**, and deliberately so: a redemption cannot be undone and the credits are scarce, so nothing spends one until you switch this on. The switch is **fleet-wide** because the policy is — a credit is spent only when the weekly (never the 5-hour) window is exhausted **and** either every other Codex account is unavailable too, or the credit itself expires within three days. Toggled from the settings screen (**g** → Auto-redeem) and applied live on reload, so it can be armed or killed without a restart. Individual accounts can opt out with `accounts[].autoRedeemReset` below |
...
| `accounts[].headersTimeoutMs` | How long (ms) to wait for this account's upstream to send response **headers** before the socket is treated as dead and the request fails fast (positive integer). The body is never subject to it. Overrides the fleet default (`TEAMCLAUDE_UPSTREAM_HEADERS_TIMEOUT_MS`, else 120000) for this account only — for slow backends such as a Codex sidecar, whose non-streaming replies can take minutes to first byte. Applied live on reload; `0`, negative, fractional or non-numeric values are ignored |
...
| `TEAMCLAUDE_HEADERLESS_429_RETRY_DELAY_MS` | `2000` | How long to wait before the single retry of a **headerless** (request-scoped) 429 — one carrying no `retry-after` and no `anthropic-ratelimit-*` headers. Retried once per request, on the account it landed on, and only for a caller that waits on the connection (a Codex caller is answered at once instead); if the retry is refused too the 429 goes to the client, just later. Lower it only if you would rather have the 429 sooner than have the transient absorbed — see [the two kinds of 429](routing.md#the-two-kinds-of-429) |
