...
| `customModels` | Optional list of third-party models to register with Claude Code at launch — see [OpenAI models](openai.md). Objects: `{ model, label?, description?, contextTokens? }`. `run` injects `/model` picker rows (`--settings`) and one dispatchable subagent per model (`--agents`); both `run` and `env` set `CLAUDE_CODE_MAX_CONTEXT_TOKENS` to the largest `contextTokens` and register the first entry via `ANTHROPIC_CUSTOM_MODEL_OPTION` |
| `customModelAgents` | Set to `false` to stop `run` injecting the per-model subagents (`--agents`). Use it when you keep your own agents in `~/.claude/agents/`: their `model:` ids still reach the proxy, and the plain agents only add listing noise and invite a dispatch with no pinned effort. Picker rows and env vars are unaffected. Default `true` |
